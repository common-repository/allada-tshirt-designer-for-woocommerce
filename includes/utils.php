 ...
 */
function atd_o_admin_fields( $options ) {
	...
	foreach ( $options as $value ) {
		...
		$option_value    = '';
		$url_field_value = '';
		$raw_hierarchy   = atd_explode_x( array( '[', ']' ), $value['name'] );
		$hierarchy       = array_filter( $raw_hierarchy );
		$section_types   = array( 'sectionbegin', 'sectionend' );
		$settings_table  = atd_get_proper_value( $options[0], 'table', 'metas' );
		...
		if ( ! in_array( $value['type'], $section_types, true ) & ! empty( $hierarchy ) ) {
			...
			$option_value = atd_get_proper_value( $_SESSION['o-data'], $session_key, false );
			...
			$session_key = $root_key . "_$post_id";
			$root_value  = atd_get_proper_value( $_SESSION['o-data'], $session_key, false );
			if ( $root_key !== $value['name'] ) {
				$option_value = atd_o_find_in_array_by_key( $root_value, $value['name'] );
			}
		}
		...
		switch ( $value['type'] ) {
			...
			case 'post-type':
				if ( 'post-type' === $value['type'] ) {
					...
					$value['args']['posts_per_page'] = atd_get_proper_value( $value['args'], 'posts_per_page', -1 );
					$posts                           = get_posts( $value['args'] );
					$posts_ids                       = atd_get_proper_value( $value, 'first_value', array() );
			...
			case 'checkbox':
				...
				if ( ! empty( $value['title'] ) ) {
					...
					$cb_value = atd_get_proper_value( $value, 'value', false );
				if ( ! $cb_value ) {
					$cb_value = atd_get_proper_value( $value, 'default', 1 );
				}
				?>
			...
			case 'image':
				$set_btn_label    = atd_get_proper_value( $value, 'set', 'Set image' );
				$remove_btn_label = atd_get_proper_value( $value, 'remove', 'Remove image' );

				$img_src      = '';
				$root_img_src = atd_o_get_proper_image_url( $option_value, false );
				if ( $root_img_src ) {
					$img_src = atd_o_get_medias_root_url( "/$root_img_src" );
				}
				?>
				...
				break;
			case 'file':
				$set_btn_label    = atd_get_proper_value( $value, 'set', 'Set file' );
				$remove_btn_label = atd_get_proper_value( $value, 'remove', 'Remove file' );
				?>
								<div class="<?php echo esc_attr( $value['class'] ); ?>">
					<button class="button o-add-media"><?php echo wp_kses_post( $set_btn_label ); ?></button>
					<button class="button o-remove-media"><?php echo wp_kses_post( $remove_btn_label ); ?></button>
			...
			case 'repeatable-fields':
				...
				$value['popup'] = atd_get_proper_value( $value, 'popup', false );

				$style = atd_get_proper_value( $value, 'style', '' );
				if ( $value['popup'] ) {
					add_thickbox();
					$modal_id = uniqid( 'o-modal-' );
					echo wp_kses_post( "<a style='$style' class='o-modal-trigger button button-primary button-large' data-toggle='o-modal' data-target='#$modal_id' data-modalid='$modal_id'>" . $value['popup_button'] . '</a>' );
					echo wp_kses_post(
				...
				?>
					...
					foreach ( $option_value as $i => $row ) {
						...
						foreach ( $value['fields'] as $field ) {
							...
							echo atd_o_admin_fields( array( $field ) );
						}
						?>
						<td>
					...
					$row_tpl                    = atd_get_row_template( $value );
					$row_tpl                    = preg_replace( "/\r|\n/", '', $row_tpl );
					$row_tpl                    = preg_replace( '/\s+/', ' ', $row_tpl );
					$tpl_id                     = uniqid();
					$o_row_templates[ $tpl_id ] = $row_tpl;

					$add_label = atd_get_proper_value( $value, 'add_btn_label', __( 'Add', 'vpc' ) );
					?>
				</tbody>
				</table>
			...
			case 'button':
				...
				break;
			case 'google-font':
				atd_o_get_google_fonts_selector( $option_value, esc_attr( $value['id'] ), esc_attr( $value['name'] ), esc_attr( $value['css'] ), esc_attr( $value['class'] ) );
				break;
 ...
 */
function atd_get_row_template( $value ) {
	...
	foreach ( $value['fields'] as $field ) {
		...
		$row_tpl                     .= atd_o_admin_fields( array( $field_tpl ) );
	}
 ...
 * @return mixed
 */
function atd_get_proper_value( $values, $search_key, $default_value = '' ) {
 ...
 * @return type
 */
function atd_explode_x( $delimiters, $string ) {
 ...
 * @return type
 */
function atd_get_media_url( $media_id ) {
 ...
 * @return type
 */
function atd_o_find_in_array_by_key( $root_value, $key ) {
 ...
 */
function atd_o_get_proper_image_url( $suspected_link, $with_root = true ) {
	...
	if ( is_numeric( $suspected_link ) ) {
		...
		$img_src     = str_replace( atd_o_get_medias_root_url( '/' ), '', $raw_img_src );
	}
	$img_src = str_replace( atd_o_get_medias_root_url( '/' ), '', $img_src );
	// Code for bad https handling.
	if ( strpos( atd_o_get_medias_root_url( '/' ), 'https' ) === false ) {
		$https_home = str_replace( 'http', 'https', atd_o_get_medias_root_url( '/' ) );
		$img_src    = str_replace( $https_home, '', $img_src );
	}

	if ( $with_root ) {
		$img_src = atd_o_get_medias_root_url( "/$img_src" );
	}
	return $img_src;
}
/**
 ...
 */
function atd_o_starts_with( $haystack, $needle ) {
	// search backwards starting from haystack length characters from the end.
 ...
 * @return type
 */
function atd_o_ends_with( $haystack, $needle ) {
	// search forward starting from end minus needle length characters.
 ...
 */
function atd_o_get_medias_root_url( $path = '/' ) {
 ...
 */
function atd_o_get_google_fonts_selector( $selected_font = false, $id = '', $name = '', $style = '', $class = '' ) {
	 ...
	 */
function atd_o_register_google_font( $font_name, $raw_url ) {
